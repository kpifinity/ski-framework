...
|---|---|---|
| v3.0 | 0.6.0 | Sovereignty (L3): added `test_manipulation_resistant_verdict.py` — threat T9 (adversarial/injected telemetry), structural check that the verifier grounds assertions against the measurement and cited obligation, recomputes `satisfied` mechanically, and that risk-tier policy downgrades any disagreement. Functional proof in the runtime suite (`v3/tests/test_adversarial_telemetry.py`): a scripted FakeLLM simulates a successful injection and a real breach is never CLEARed. Seven checks now runnable. |
